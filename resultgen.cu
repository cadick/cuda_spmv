...
// nvcc -std=c++11 resultgen.cu -o resultgen -I/usr/local/cuda/targets/x86_64-linux/include/ -L/usr/local/cuda/targets/x86_64-linux/lib/ -lcufft
// ./resultgen

// Windows compiling:
// nvcc resultgen.cu -o resultgen -I/usr/local/cuda/targets/x86_64-linux/include/ -L/usr/local/cuda/targets/x86_64-linux/lib/ -lcufft
// resultgen.exe
...
int main() {
    for (int zeros = 0; zeros < 16; zeros++) {
        for (int iteration = 0; iteration < 3; iteration++) {
            ...
            fillStructuredMatrix(h_m_W, dimU * M, dimB, zeros);
            //printMatrices(h_m_W, MATRIX_SIZE, dimB, M);
